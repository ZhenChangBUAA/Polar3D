...
def normalize_mesh(mesh, method="unit_cube_centered", target_size=1.0):
    """
    归一化trimesh加载的Mesh对象

    参数:
        mesh: trimesh.Trimesh对象 - 待归一化的Mesh
        method: str - 归一化方法
                - "unit_cube_centered": 缩放到[-1,1]立方体，中心在原点（默认）
                - "unit_cube_origin": 缩放到[0,1]立方体，原点在左下角
                - "custom_cube": 缩放到指定边长的立方体，中心在原点
                - "unit_sphere": 缩放到单位球体（半径=1），中心在原点
        target_size: float - 自定义立方体边长/球体半径（仅method为custom_cube/unit_sphere时生效）

    返回:
        trimesh.Trimesh对象 - 归一化后的Mesh（原Mesh不会被修改）
    """
    # 复制Mesh，避免修改原对象
    ...
    if len(vertices) == 0:
        raise ValueError("Mesh没有顶点数据！")

    # 1. 通用步骤：计算原始包围盒和中心点
    min_bound = vertices.min(axis=0)  # 包围盒最小值
    max_bound = vertices.max(axis=0)  # 包围盒最大值
    center = (min_bound + max_bound) / 2  # 几何中心点
    scale_factor = 1.0  # 默认缩放因子

    if method == "unit_cube_centered":
        # 方法1：缩放到[-1,1]立方体，中心在原点
        # 平移到原点
        normalized_mesh.apply_translation(-center)
        # 计算最大尺寸（包围盒对角线的一半）
        max_dim = np.max(max_bound - min_bound) / 2
        scale_factor = 1.0 / max_dim if max_dim != 0 else 1.0
        normalized_mesh.apply_scale(scale_factor)

    elif method == "unit_cube_origin":
        # 方法2：缩放到[0,1]立方体，原点在左下角
        # 平移到左下角为原点
        normalized_mesh.apply_translation(-min_bound)
        # 缩放到[0,1]范围
        max_dim = np.max(max_bound - min_bound)
        scale_factor = 1.0 / max_dim if max_dim != 0 else 1.0
        normalized_mesh.apply_scale(scale_factor)

    elif method == "custom_cube":
        # 方法3：缩放到指定边长的立方体，中心在原点
        # 平移到原点
        normalized_mesh.apply_translation(-center)
        # 缩放到指定边长
        max_dim = np.max(max_bound - min_bound) / 2
        ...
        # 方法4：缩放到单位球体（半径=target_size），中心在原点
        # 平移到原点
        normalized_mesh.apply_translation(-center)
        # 计算所有顶点到原点的最大距离（原始球体半径）
        max_radius = np.max(np.linalg.norm(normalized_mesh.vertices, axis=1))
        scale_factor = target_size / max_radius if max_radius != 0 else 1.0
        normalized_mesh.apply_scale(scale_factor)

    else:
        raise ValueError(
            f"不支持的归一化方法：{method}，可选值：unit_cube_centered, unit_cube_origin, custom_cube, unit_sphere")

    # 返回归一化后的Mesh，并输出信息
    # print(f"归一化完成（方法：{method}）")
    # print(f"  缩放因子: {scale_factor:.6f}")
    # print(f"  归一化后包围盒范围:")
    # print(f"    最小值: {normalized_mesh.vertices.min(axis=0)}")
    # print(f"    最大值: {normalized_mesh.vertices.max(axis=0)}")
    # print(f"  归一化后中心点: {np.mean(normalized_mesh.vertices, axis=0).round(6)}")

    return normalized_mesh


# ================= 核心处理逻辑封装 (Process Function) =================

def process_single_mesh(input_mesh_path, output_root_path, args):

    # print(f"\n Processing: {input_mesh_path}")

    # 构造输出目录: output_root/filename_without_ext
    mesh_basename = os.path.basename(input_mesh_path).split('.')[0]
    output_path = os.path.join(output_root_path, mesh_basename)

    # 如果已经存在且不强制覆盖，可以选择跳过 这里默认覆盖)
    ...
    # 1. 第一次归一化
    mesh = normalize_mesh(mesh, method="unit_cube_centered")
    ...
    # Volume points limited by bounds
    volume_rand_points = np.random.uniform(-args.bounds, args.bounds, size=(args.num_vlume_points, 3))

    # Compute SDF
    f = SDF(mesh.vertices, mesh.faces)
    near_surface_points_with_sdf = np.concatenate([near_surface_points, f(near_surface_points)[:, np.newaxis]], axis=1)
    volume_rand_points_with_sdf = np.concatenate([volume_rand_points, f(volume_rand_points)[:, np.newaxis]], axis=1)
    ...
    # 准备 scalar bounds
    bounds_scalar = np.array(args.bounds, dtype=np.float32)

    save_dict = {
        'surface': surface.astype(np.float32),
        'near_surface_points': near_surface_points_with_sdf.astype(np.float32),
        'volume_rand_points': volume_rand_points_with_sdf.astype(np.float32),
        'bounds': bounds_scalar
    }

    if args.sample_sharp_edge:
        print("  - Sampling sharp edges...")
        try:
            # 注意：传入 watertight mesh 的路径
            sharp_surface = sharp_edge_sampling(
                f"{output_path}/watertight_mesh.obj",
                num_views=args.num_sharp_surface_points,
                sharpness_threshold=math.radians(args.angle_threshold)
            )
            if sharp_surface is not None:
                save_dict['sharp_surface'] = sharp_surface.astype(np.float32)
        except Exception as e:
            print(f"  ⚠️ Warning: Sharp edge sampling failed for {mesh_basename}: {e}")

    # Save
    np.savez(f'{output_path}/samples.npz', **save_dict)
    print(f"  ✅ Saved to {output_path}/samples.npz")

    torch.cuda.empty_cache()

if __name__ == "__main__":
    parser = ArgumentParser(description="Watertight mesh and sampling points")
    parser.add_argument("--input_dir", type=str, default="/root/dataset/Objaverse-3D/glbs/000-000/",
                        help="Directory containing input mesh files")
    parser.add_argument("--input_mesh", type=str, default="", help="Path to the input mesh file")
    parser.add_argument("--output_path", type=str, default="/root/dataset/Objaverse-3D/npz/000-000", help="Path to save the watertight mesh and sampled points")
    ...
    # 1. 确定要处理的文件列表
    if args.input_mesh and os.path.isfile(args.input_mesh):
        # 模式 A: 单文件模式
        files_to_process.append(args.input_mesh)
    elif os.path.isdir(args.input_dir):
        # 模式 B: 目录批量模式
        print(f"Scanning directory: {args.input_dir} ...")
        file_pattern = os.path.join(args.input_dir, "*.glb")
        matched_glb_files = glob(file_pattern, recursive=False)
        files_to_process = sorted(matched_glb_files)
    else:
        print("Error: Please provide a valid --input_dir or --input_mesh")
        exit(1)
    ...
    for i, file_path in enumerate(files_to_process):
        print(f"[{i + 1}/{len(files_to_process)}] Starting...")
        try:
            process_single_mesh(file_path, args.output_path, args)
            success_count += 1
        except Exception as e:
    ...
    #         device=args.device,
    #     )
    # mesh = normalize_mesh(mesh, method="unit_cube_centered") # Second Normalization
    # # Save the watertight mesh
    ...
    # normals = mesh.face_normals[faces]
    # surface = np.concatenate([surface_points, normals], axis=1)
    # # 这样保存进去就是 shape=() 的标量，而不是 shape=(2,) 的数组
    # bounds_scalar = np.array(args.bounds, dtype=np.float32)
    # if args.sample_sharp_edge:
    ...
    #         surface=surface.astype(np.float32), # (num_surface_points, 6), surface points with normals
    #         sharp_surface=sharp_surface.astype(np.float32), # (num_sharp_surface_points, 6), sharp surface points with normals
    #         near_surface_points=near_surface_points_with_sdf.astype(np.float32), # (num_near_surface_points, 4), sampled points near the surface with SDF values
    #         volume_rand_points=volume_rand_points_with_sdf.astype(np.float32), # (num_vlume_points, 4), sampled points in the volume within bounds with SDF values
    #         bounds=bounds_scalar.astype(np.float32)
    #     )
    # else:
    #     # Save the samples
    #     np.savez(
