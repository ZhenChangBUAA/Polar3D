...
class FluxTransformer1DModel(ModelMixin, ConfigMixin, PeftAdapterMixin):
    ...
    def forward(
        ...
        if USE_PEFT_BACKEND:  # 只有peft和transformers都满足兼容版本时，才启用 PEFT 后端
        ...
        _, N, _ = hidden_states.shape  # 只要三维信息
...
class FluxDenoiser(BaseModule):
    ...
    def configure(self) -> None:
        ...
        if self.cfg.pretrained_model_name_or_path:
            ...
            #     weights_only=True,
            # )
            # 修改by cz 适配safetensors格式
            ckpt = load_file(self.cfg.pretrained_model_name_or_path, device="cpu")
