...
def main(args, extras) -> None:
    ...
    # def set_system_status(system, resume_path):
    #     if resume_path and os.path.exists(resume_path):
    #         # 分布式 checkpoint 是目录，用 strategy 加载，而非 torch.load
    #         if os.path.isdir(resume_path):
    #             # 用 trainer 的 strategy 加载分布式 checkpoint
    #             trainer.strategy.load_checkpoint(resume_path)
    #             print(f"成功从分布式 checkpoint 目录恢复: {resume_path}")
    #         # 单个文件 checkpoint，用 torch.load
    #         else:
    #             ckpt = torch.load(resume_path, map_location="cpu", weights_only=True)  # 加上 weights_only=True 消除警告
    #             # 你的原有加载逻辑
    #             system.load_state_dict(ckpt["state_dict"])
    #             print(f"成功从单个文件恢复 checkpoint: {resume_path}")
    #     else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", default="/root/Polar3D-2/configs/train-geometry-diffusion/step1x-3d-geometry-1300m.yaml", help="path to config file")
    parser.add_argument(
        "--gpu",
        default="0,1,2,3",
    ...
    group.add_argument("--train", action="store_true", default=True, help="执行训练（默认）")
    group.add_argument("--validate", action="store_true", help="执行验证")
    group.add_argument("--test", action="store_true", help="执行测试")
    group.add_argument("--export",action="store_true", help="导出模型")
    ...
    parser.add_argument('--local_rank', type=int, default=0, help='DeepSpeed会自动传入这个参数，必须接收它')
