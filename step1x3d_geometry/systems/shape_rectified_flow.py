...
class RectifiedFlowSystem(BaseSystem):
    ...
    def forward(self, batch: Dict[str, Any], skip_noise=False) -> Dict[str, Any]:
        ...
        if "sharp_surface" in batch.keys():
            ...
            sharp_surface = None
        w, latents, _ = self.shape_model.encode(
        ...
        if self.cfg.visual_condition_type is not None:
            ...
            if "image" in batch and batch["image"].dim() == 5:
                ...
                print("recitify这里没有AOLP")
            else: # 走这里
                visual_cond = self.visual_condition(batch).to(latents)
                if "aolp" in batch:

                    aolp_data = batch["aolp"]
                    if aolp_data.shape[1] == 1:
                        aolp_data = aolp_data.repeat(1, 3, 1, 1)
                    original_rgb = batch["image"]
                    # print("aolp范围：", aolp_data.min(), aolp_data.max())
                    # print("image范围：", original_rgb.min(), original_rgb.max())
                    batch["image"] = aolp_data
                    aolp_latents = self.visual_condition(batch).to(latents)
                    batch["image"] = original_rgb
                    visual_cond = (visual_cond + aolp_latents) / 2.0
        ...
        ).sample  # caption_cond is not None
    ...
    def sample(
        ...
        **kwargs,
        ...
        if "image" in sample_inputs:
            ...
            # ==========================引入 AOLP as condition====================================
            if "aolp" in sample_inputs:
                sample_inputs['aolp'] = [
                    Image.open(img).convert("RGB") if isinstance(img, str) else img
                    for img in sample_inputs["aolp"]
                ]
                sample_inputs['aolp'] = preprocess_image(sample_inputs['aolp'], **kwargs)
                cond_aolp = self.visual_condition.encode_image(sample_inputs['aolp'])

                # 3. 核心融合：Latent 相加求均值
                # 此时 cond_rgb 和 cond_aolp 形状必须一致
                cond = (cond + cond_aolp) / 2.0

            else:
                # 如果没有提供 aolp 数据，回退到仅使用 RGB
                print("=====================❌ No Aolp Input========================")
        ...
        for _ in range(sample_times):
            ...
            print("use XXX_cond but not cond")
            print(visal_cond.type())
